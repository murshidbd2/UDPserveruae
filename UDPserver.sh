...
idioam_lang(){
  ...
  for arch in $listarq; do
    if ! wget -O $lang_dir/${list_lang[$lg]}/$arch "https://raw.githubusercontent.com/murshidbd2/UDPserveruae/main/lang/${list_lang[$lg]}/$arch" &>/dev/null ;then
...
if [[ ! -e $udp_file/UDPserver.sh ]]; then
  ...
  check_sistem
	...
	wget -O $udp_file/limitador.sh "#!/bin/bash
...
# 0 1 * * * /etc/ADMRufu/install/limitador.sh >> /var/spool/cron/crontabs/root" &>/dev/null
...
menu_udp(){
	...
	print_center -ama 'UDPserver Binary by MB TUNNE ADMIN WHATSAPP +8801301099152'
